...
const credentials = JSON.parse(fs.readFileSync(__dirname+ "/credentials/credentials.json"));
const token = fs.readFileSync(__dirname + "/credentials/token.json");
...
module.exports = {
     ...
     */
    send:async (to,link,callback)=>{
        if(typeof callback == "function"){
            oauth2Client = authorize();
            email = createEmail(to,link);
